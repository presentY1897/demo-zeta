...
- **E2E(Playwright)는 T3 병합 후로 미뤘다.** `e2e/` 기반이 T3에서 만들어지는 중이라 이 브랜치에는
  아직 없다. 위 "테스트"의 E2E 항목은 체크하지 않은 채로 두었고, 시나리오(오피스 로그인 → 작성 →
  웹 반영 → 고정 → 배너 교체 → 삭제 → 미노출)는 이번에 curl로 같은 순서를 수동 검증했다.
