...
### CI 도입 중 드러난 경합 (09-02)

CI에서 **빈 테스트 DB**로 처음 돌리자 "초기화해도 지운 대화가 되살아나지 않는다"가 깨졌다.
`expectedSeq` 가드에 틈이 있었다 — 채번(`max(seq)` 조회)과 INSERT 커밋 사이에 다른 트랜잭션의
삭제가 끼어들면, 조회 시점에는 자리가 맞아 통과하고 커밋은 삭제 뒤에 이뤄져 **지운 대화 뒤에
응답이 남는다**(`[0, 2]`). READ COMMITTED에서 자연스럽게 생기는 구멍이다.

`pg_advisory_xact_lock`으로 **방 단위 쓰기를 직렬화**해 닫았다. 메시지 추가와 잘라내기가 같은
잠금을 잡으므로 둘이 겹치지 않는다(트랜잭션 종료 시 자동 해제). 가드는 그대로 두었다 —
잠금은 순서를 보장하고, 가드는 "내 자리가 아직 있는가"를 판정한다.

### CI가 잡은 두 번째 결함 — 중단 응답이 두 번 남는다 (09-02)

문서만 바꾼 커밋에서 CI가 실패했다. 코드가 같은데 직전 실행은 통과했으니 비결정적이라는 뜻이었고,
실패는 `strict mode violation: '⏹ 응답이 중단됐어요' resolved to 2 elements` — **중단 응답이 두 번**
남아 있었다.

중단 라우트가 "잘라내기 → 넣기"를 **두 트랜잭션으로** 하고 있었다. 그 사이가 열려 있으면:

1. 중단 신호가 `afterSeq` 뒤를 지운다 (커밋)
2. 그 틈에 `/api/chat`의 늦은 저장이 `expectedSeq = afterSeq + 1`을 확인 → 자리가 비어 있으니 넣는다
3. 중단 신호가 이어서 자기 응답을 넣는다 → **두 개**

방 잠금은 각 연산을 직렬화했지만 두 연산 사이를 덮지 못했다. `replaceTail`로 **한 트랜잭션 안에서**
잠금을 쥔 채 지우고 넣도록 바꿔 닫았다. 이제 늦은 저장은 이 트랜잭션 앞이나 뒤에만 놓이고,
뒤에 놓이면 자리가 어긋나 `expectedSeq` 가드에 걸린다.

**회귀 감지는 E2E가 맡는다**(중단 표시가 정확히 1개인지 확인). 깨진 코드로 로컬에서 3회 중 1회
재현되는 확률적 검출이라, 통합 테스트로 순서를 강제하려 했지만 핸들러 왕복 때문에 늘 안전한 순서로만
흘러 **깨진 코드에서도 통과했다.** 그런 테스트는 남기지 않았다 — 대신 정의된 두 순서(늦은 저장이
먼저 / 중단이 먼저)를 각각 결정적으로 검증하는 기존 테스트를 유지하고, 원자성은 구조로 보장한다.

같은 로그에서 **E2E 서버가 마이그레이션 전에 부팅**해 `relation "daily_metrics" does not exist`를
뱉고 있던 것도 발견했다. Playwright는 webServer를 먼저 띄우고 globalSetup을 돌린다. DB 준비를
`pnpm e2e`가 먼저 실행하는 `e2e/prepare-db.ts`로 옮겨 순서를 뒤집었다.
